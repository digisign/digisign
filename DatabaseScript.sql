...
CREATE VIEW `digi`.`institution_contact` AS
    SELECT 
    ...
    FROM
        ((`digi`.`institution` `it`
        JOIN `digi`.`contact` `co` ON ((`it`.`Contact_Id` = `co`.`Contact_Id`)))
        JOIN `digi`.`contact_address` `ca` ON ((`ca`.`Contact_Id` = `co`.`Contact_Id`)));
        
       
	CREATE VIEW `digi`.`institution_course_details` AS
    SELECT 
    ...
    FROM
        (((`digi`.`institution_user` `iu`
        JOIN `digi`.`contact` `cn` ON ((`iu`.`Contact_Id` = `cn`.`Contact_Id`)))
        JOIN `digi`.`institution` `it` ON ((`it`.`Contact_Id` = `cn`.`Contact_Id`)))
        JOIN `digi`.`course` `co` ON ((`it`.`Institution_Id` = `co`.`Institution_Id`)));
        
        CREATE VIEW `digi`.`institution_user_details` AS
    SELECT 
    ...
    FROM
        (((`digi`.`institution_user` `iu`
        JOIN `digi`.`contact` `co` ON ((`iu`.`Contact_Id` = `co`.`Contact_Id`)))
        JOIN `digi`.`institution` `it` ON ((`it`.`Contact_Id` = `co`.`Contact_Id`)))
        JOIN `digi`.`contact_address` `ca` ON ((`co`.`Contact_Id` = `ca`.`Contact_Id`)))
        JOIN `digi`.`subject1` `sb` ON ((`co`.`Course_Id` = `sb`.`Course_Id`));
        
        CREATE VIEW `digi`.`learner_marks` AS
    SELECT 
    ...
    FROM
        (((((`digi`.`learner` `ln`
        JOIN `digi`.`contact` `cn` ON ((`ln`.`Contact_Id` = `cn`.`Contact_Id`)))
        JOIN `digi`.`learner_credential` `lc` ON ((`lc`.`Learner_Id` = `ln`.`Learner_Id`)))
        JOIN `digi`.`course` `co` ON ((`lc`.`Course_Id` = `co`.`Course_Id`)))
        JOIN `digi`.`subject1` `sb` ON ((`co`.`Course_Id` = `sb`.`Course_Id`)))
        JOIN `digi`.`subject_marks` `sm` ON ((`sb`.`Subject_Id` = `sm`.`Subject_Id`)));
        
        CREATE VIEW `digi`.`learner_user_details` AS
    SELECT 
    ...
    FROM
        (((`digi`.`user` `us`
        JOIN `digi`.`learner` `lr` ON ((`us`.`User_Id` = `lr`.`User_Id`)))
        JOIN `digi`.`contact` `co` ON ((`lr`.`Contact_Id` = `co`.`Contact_Id`)))
        JOIN `digi`.`contact_address` `ca` ON ((`co`.`Contact_Id` = `ca`.`Contact_Id`)));
        
        CREATE VIEW `digi`.`requester_credential` AS
    SELECT 
    ...
    FROM
        (((((`digi`.`requester_user` `rt`
        JOIN `digi`.`contact` `co` ON ((`rt`.`Contact_Id` = `co`.`Contact_Id`)))
        JOIN `digi`.`request` `ru` ON ((`ru`.`Requester_Id` = `rt`.`Requester_Id`)))
        JOIN `digi`.`learner_credential` `lc` ON ((`lc`.`Learner_Id` = `ru`.`Learner_Id`)))
        JOIN `digi`.`credential` `cr` ON ((`cr`.`Course_Id` = `lc`.`Course_Id`)))
        JOIN `digi`.`institution` `it` ON ((`it`.`Institution_Id` = `cr`.`Institution_Id`)));
        
        CREATE VIEW `digi`.`requester_user_details` AS
    SELECT 
        `us`.`User_Id` AS `User_Id`,
        ...
        `ca`.`Postal_Code` AS `Postal_Code`
    FROM
        (((`digi`.`requester_user` `ru`
        JOIN `digi`.`user` `us` ON ((`ru`.`User_Id` = `us`.`User_Id`)))
        JOIN `digi`.`contact` `co` ON ((`ru`.`Contact_Id` = `co`.`Contact_Id`)))
        JOIN `digi`.`contact_address` `ca` ON ((`co`.`Contact_Id` = `ca`.`Contact_Id`)))